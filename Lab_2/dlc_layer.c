...
window_open(DLC_Conn_Info_TYPE *dci)
{
 ...
 result = (* dci).snd_nxt - (* dci).snd_una;
 if(result < 0){ 
  result *= -1;
 }
 int window_size = (* dci).window_size;
 if( window_size != result && DataInPDUBuffer() > 0 ){
  result = 1;
 }else {
  result = 0;
 }
				  ...
				  DLC_Conn_Info_TYPE *dci)
{
     /* Check the address and if not correct discard frame and
         return 0;   */
  if( pdu->u.d_pdu.address != GetNodeID()){
    return 0;
  }
     /* Otherwise:  
			  ...
			  DLC_Conn_Info_TYPE *dci)
{
	...
	d_pdu->u.d_pdu.address = GetRecieverID(dlc_layer_entity);
